...
from .object_detection_2d_geometric_ops import ResizeRandomInterp, RandomFlip, RandomRotate
from .object_detection_2d_image_boxes_validation_utils import BoundGenerator, BoxFilter, ImageValidator

def random_blur_dropout(chip, value, p_dropout, p_blur, max_length=None):
    chip = chip.copy()
    i_shape, j_shape, k_shape = chip.shape
    if max_length is None:
        random_length = np.random.randint(min(i_shape, j_shape))
    else:
        if max_length > i_shape or max_length > j_shape:
            raise Exception('dropout size {} larger than chip shape {}'.format(
                max_length, (i_shape, j_shape)))
        random_length = np.random.randint(max_length)
    ml = int(random_length/4)
    while np.random.rand() < p_dropout:
        x_c = np.random.randint(low=ml, high=(i_shape - ml))
        y_c = np.random.randint(low=ml, high=(j_shape - ml))
        chip[(x_c - ml):(x_c + ml), (y_c - ml):(y_c + ml), :] = value
    while np.random.rand() < p_blur:
        x_c = np.random.randint(low=ml, high=(i_shape - ml))
        y_c = np.random.randint(low=ml, high=(j_shape - ml))
        replace_value = np.mean(chip[(x_c-ml):(x_c+ml), (y_c-ml, y_c+ml), :])
        chip[(x_c-ml):(x_c+ml), (y_c-ml):(y_c+ml), :] = replace_value
    return chip

class SSDRandomBlurDropout:
    
    def __init__(self, blur_prob, dropout_prob, mean_color, labels_format={'class_id': 0, 'xmin': 1, 'ymin': 2, 'xmax': 3, 'ymax': 4}):
    ...
    def __call__(self, image, labels=None, return_inverter=False):
        changed_image = random_blur_dropout(image, value=0, p_dropout=self.dropout_prob, p_blur=self.blur_prob)
        def inverter():
            saved_image = image
            return saved_image
        if return_inverter:
            return changed_image, labels, inverter
        return changed_image, labels
...
class SSDDataAugmentation:
    ...
    def __init__(self,
        ...
        self.random_rotate = RandomRotate(angles=[90, 180, 270], prob=0.5, labels_format=self.labels_format)
        self.random_dropout_blur = SSDRandomBlurDropout(blur_prob=0.1,
                                                        dropout_prob=0.1,
                                                        mean_color=170 if background is None else background[0],
                                                        labels_format={'class_id': 0, 'xmin': 1, 'ymin': 2, 'xmax': 3, 'ymax': 4})
