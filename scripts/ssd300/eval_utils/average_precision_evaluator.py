...
class Evaluator:
    ...
    def __init__(self,
        ...
        self.apply_adjacent_rule = apply_adjacent_rule
        self.apply_unique_box = apply_unique_box
    ...
    def predict_on_dataset(self,
        ...
        for j in tr:
            # Generate batch.
            batch_X, batch_image_ids, batch_eval_neutral, batch_inverse_transforms, batch_orig_images, batch_orig_labels = next(generator)
            # Predict.
            ...
            for k, batch_item in enumerate(y_pred):
                ...
                for box in batch_item:
                    ...
                    if label_convert is not None:
                        class_id = label_convert[class_id]
                    ...
                    xmin = max(round(box[xmin_pred], 1), 0)
                    ymin = max(round(box[ymin_pred], 1), 0)
                    xmax = min(round(box[xmax_pred], 1), batch_orig_images[k].shape[1])
                    ymax = min(round(box[ymax_pred], 1), batch_orig_images[k].shape[0])
                    if not self.apply_unique_box:
                        results[class_id].append((image_id, confidence, xmin, ymin, xmax, ymax))
                    if class_id not in max_class:
                        max_class[class_id] = (image_id, confidence, xmin, ymin, xmax, ymax)
                    else:
                        if confidence > max_class[class_id][1]:
                            max_class[class_id] = (image_id, confidence, xmin, ymin, xmax, ymax)
                if self.apply_adjacent_rule:
                    class_keys = list(max_class.keys())
                    for c1 in class_keys:
                        ...
                        for c2 in class_keys:                      
                            if ((c1 == (c2 + 1)) or (c1 == (c2 - 1))):
                                aj = True
                                break
                        if not aj and max_class[c1][1] < 0.7:
                            del(max_class[c1])
                if self.apply_unique_box:
                    for ci in max_class:
                        results[ci].append(max_class[ci])
    ...
    def match_predictions(self,
        ...
        for class_id in range(1, self.n_classes + 1):
            ...
            iou_var = np.zeros(len(predictions))
            ...
            preds_data_type = np.dtype([('image_id', 'object'),
                                        ('confidence', 'f4'),
                                        ('xmin', 'f4'),
                                        ('ymin', 'f4'),
                                        ('xmax', 'f4'),
...
#                 prediction = predictions_sorted[i]
            ...
            true_positives.append(true_pos)
            false_positives.append(false_pos)
            false_negatives.append(gt_matched)
            overlap_iou.append(iou_var)
        ...
        if ret:
            return true_positives, false_positives, cumulative_true_positives, cumulative_false_positives, false_negatives
